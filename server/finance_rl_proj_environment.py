...
class TradingEnv(Environment):
    ...
    def LLM_score(self,llm_output):
        ...
        match = re.search(r"(?:Score|Reward|Alignment|Timing):\s*([\d\.-]+)", str(llm_output))
        if match:
            try:
    ...
    def step(self, action: TradingAction) -> TradingObservation:
        ...
        raw_pnl = (action.quantity * (action.price - current_price['Close'])) if action.action_type == 'sell' else 0.0
        pnl_reward = (np.tanh(raw_pnl / 1000) + 1) / 2

        #AI graders
        stock_analyzer= stock_analysis(stock_name=action.stock_symbol, start_date="2020-01-01", end_date="2024-01-01")
        fundamental_data= stock_analyzer.fundamental_analysis(action_taken=action.action_type)
        technical_data= stock_analyzer.technical_analysis(action_taken=action.action_type)
